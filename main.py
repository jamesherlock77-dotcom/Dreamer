...
async def testupdate(ctx: commands.Context):
    """Posts a test message immediately and shows the currently detected versions."""
    ...
    await ctx.send(f"📦 Current versions — Live: `{live}` | Dev: `{dev}`")
...
async def check_for_updates(force_channel: Optional[discord.abc.Messageable] = None) -> None:
    ...
    if live is not None and live != state.get("live_version"):
        changed_messages.append(f"🚀 **Live version updated:** `{state.get('live_version')}` → `{live}`")
        state["live_version"] = live

    if dev is not None and dev != state.get("dev_version"):
        changed_messages.append(f"🧪 **Dev version updated:** `{state.get('dev_version')}` → `{dev}`")
